    ...
    for (let therapistIndex = 0; therapistIndex < therapists.length; therapistIndex++) {
      ...
      for (let clientIndex = 0; clientIndex < clientsToCreate; clientIndex++) {
        try {
          ...
          // Submit using evaluateHandle to avoid version issues
          const submitHandle = await page.evaluateHandle(() => {
            ...
            return buttons.find(b =>
              ...
              b.innerText.includes('שמירה') || // Hebrew support
              b.innerText.includes('הוספה')    // Hebrew support
            );
          });
      ...
      try {
        const logoutHandle = await page.evaluateHandle(() => {
          ...
          return buttons.find(b =>
            b.innerText.includes('Logout') ||
            b.innerText.includes('Sign Out') ||
            b.href?.includes('logout')
          );
        });

        if (logoutHandle.asElement()) {
          await logoutHandle.asElement().click();
          await new Promise(r => setTimeout(r, 2000));
        }
      } catch (error) {
