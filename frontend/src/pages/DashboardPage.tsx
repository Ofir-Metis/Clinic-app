...
import { fetchAppointments, fetchNotes, fetchStats, fetchClientCount } from '../api/dashboard'
...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    // Use coachId from auth response if available, otherwise fallback to parsing id (legacy)
    const coachId = user?.coachId || (user?.id ? parseInt(user.id) : undefined);
    console.log('Fetching dashboard data for coachId:', coachId);

    Promise.all([
      fetchAppointments(coachId),
      fetchNotes(coachId),
      fetchStats(coachId),
      fetchClientCount(coachId)
    ])
      .then(([a, n, s, c]) => {
        setAppointments(a)
        setNotes(n)
  ...
  // Handler to refresh appointments after scheduling
  const handleAppointmentScheduled = (appointment: any) => {
    // Refresh appointments list
    // Refresh appointments list
    const coachId = user?.coachId || (user?.id ? parseInt(user.id) : undefined);
    fetchAppointments(coachId).then(setAppointments);
  };
  ...
  return (
    <WellnessLayout
      ...
      showFab={true}
      fabIcon={<AddIcon />}
      fabAction={() => setAppointmentModalOpen(true)}
                ...
                <Card sx={{
                  ...
                  <CardContent sx={{ p: { xs: 3, sm: 4 } }}>
                    ...
                    <Typography variant="body1" color="text.secondary" sx={{ mb: 3, fontSize: '1.1rem' }}>
                      {selectedDate.toLocaleDateString(language === 'he' ? 'he-IL' : language === 'es' ? 'es-ES' : 'en-US', {
                        weekday: 'long',
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric'
