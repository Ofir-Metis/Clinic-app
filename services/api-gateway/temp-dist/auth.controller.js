...
let AuthController = (() => {
    ...
    var AuthController = _classThis = class {
        ...
        async register(body) {
            try {
                const response = await (0, rxjs_1.firstValueFrom)(this.httpService.post('http://auth-service:3000/auth/register', body, { withCredentials: true }));
                return response.data;
            }
            catch (error) {
                throw new common_1.HttpException(error.response?.data || 'Auth service error', error.response?.status || common_1.HttpStatus.BAD_GATEWAY);
            }
        }
        async login(body) {
            try {
                const response = await (0, rxjs_1.firstValueFrom)(this.httpService.post('http://auth-service:3000/auth/login', body, { withCredentials: true }));
                return response.data;
            }
            catch (error) {
                throw new common_1.HttpException(error.response?.data || 'Auth service error', error.response?.status || common_1.HttpStatus.BAD_GATEWAY);
    ...
    (() => {
        ...
        _register_decorators = [(0, common_1.Post)('register'), (0, throttler_1.Throttle)({ strict: { ttl: 900000, limit: 3 } })];
        _login_decorators = [(0, common_1.Post)('login'), (0, throttler_1.Throttle)({ strict: { ttl: 900000, limit: 5 } })];
