/**
 * Main application entry point for Therapists Service
...
async function bootstrap() {
  const logger = new Logger('Therapists-Service');

  try {
    logger.log('🚀 Starting Therapists Service...');
    ...
    logger.log(`🎉 Therapists Service successfully started on port ${port}`);
    logger.log(`📚 API documentation: http://localhost:${port}/docs`);
    logger.log(`🔧 Environment: ${configService.get<string>('NODE_ENV', 'development')}`);
    logger.log(`👨‍⚕️ Ready to manage coach profiles and specializations`);
    
  } catch (error) {
    logger.error('💥 Failed to start Therapists Service:', error instanceof Error ? error.stack : String(error));
    throw error;
  }
}
...
process.on('uncaughtException', (error) => {
  const logger = new Logger('TherapistsService');
